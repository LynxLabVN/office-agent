...
export const api = {
  ...
  // ── Marketing ─────────────────────────────────────────────────────────
  marketing: {
    getPipeline: (state?: string) =>
      fetchJSON<MarketingPipelineResponse>(
        `/api/marketing/pipeline/items${state ? `?state=${encodeURIComponent(state)}` : ""}`,
      ),
    movePiece: (pieceId: string, newState: string) =>
      fetchJSON<{ ok: boolean }>("/api/marketing/pipeline/move", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ piece_id: pieceId, new_state: newState }),
      }),
    getScript: (pieceId: string) =>
      fetchJSON<MarketingScript>(`/api/marketing/script?piece_id=${encodeURIComponent(pieceId)}`),
    saveScript: (pieceId: string, scriptJson: Record<string, unknown>) =>
      fetchJSON<{ ok: boolean }>("/api/marketing/script/save", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ piece_id: pieceId, script_json: scriptJson }),
      }),
    generateScript: (productSku: string, format: string) =>
      fetchJSON<MarketingScript>("/api/marketing/script/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ product_sku: productSku, format }),
      }),
    suggestHooks: (productSku: string, format: string) =>
      fetchJSON<{ hooks: string[] }>("/api/marketing/hooks/suggest", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ product_sku: productSku, format }),
      }),
    getPendingReviews: () =>
      fetchJSON<{ items: MarketingReviewItem[] }>("/api/marketing/review/pending"),
    decideReview: (pieceId: string, decision: "approve" | "revise", feedback?: string) =>
      fetchJSON<{ ok: boolean }>("/api/marketing/review/decide", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ piece_id: pieceId, decision, feedback }),
      }),
    ingestEdit: (pieceId: string, footagePath: string) =>
      fetchJSON<{ ok: boolean }>("/api/marketing/edit/ingest", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ piece_id: pieceId, footage_path: footagePath }),
      }),
    renderEdit: (pieceId: string, shotlist: string[], captions: string, music?: string) =>
      fetchJSON<{ ok: boolean; preview_url?: string }>("/api/marketing/edit/render", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ piece_id: pieceId, shotlist, captions, music }),
      }),
    previewEdit: (pieceId: string) =>
      authedFetch(`/api/marketing/edit/preview?piece_id=${encodeURIComponent(pieceId)}`),
    schedulePublish: (pieceId: string, platforms: string[], scheduledAt: string) =>
      fetchJSON<{ ok: boolean }>("/api/marketing/publish/schedule", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ piece_id: pieceId, platforms, scheduled_at: scheduledAt }),
      }),
    publishNow: (pieceId: string, platforms: string[]) =>
      fetchJSON<{ ok: boolean; urls?: Record<string, string> }>("/api/marketing/publish/now", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ piece_id: pieceId, platforms }),
      }),
    getComments: (platform?: string) =>
      fetchJSON<{ comments: MarketingComment[] }>(
        `/api/marketing/comments/list${platform ? `?platform=${encodeURIComponent(platform)}` : ""}`,
      ),
    replyToComment: (commentId: string, platform: string, text: string) =>
      fetchJSON<{ ok: boolean }>("/api/marketing/comments/reply", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ comment_id: commentId, platform, text }),
      }),
    getAnalyticsOverview: (dateFrom?: string, dateTo?: string) => {
      const qs = new URLSearchParams();
      if (dateFrom) qs.set("date_from", dateFrom);
      if (dateTo) qs.set("date_to", dateTo);
      return fetchJSON<MarketingAnalyticsOverview>(
        `/api/marketing/analytics/overview?${qs.toString()}`,
      );
    },
    getAnalyticsDrilldown: (groupBy: string, product?: string, format?: string) => {
      const qs = new URLSearchParams({ group_by: groupBy });
      if (product) qs.set("product", product);
      if (format) qs.set("format", format);
      return fetchJSON<{ rows: Record<string, unknown>[] }>(
        `/api/marketing/analytics/drilldown?${qs.toString()}`,
      );
    },
    getHooksLeaderboard: () =>
      fetchJSON<{ leaderboard: MarketingHookLeaderboardEntry[] }>(
        "/api/marketing/analytics/hooks",
      ),
    getCatalog: () =>
      fetchJSON<{ products: MarketingProduct[] }>("/api/marketing/catalog"),
    createProduct: (product: Omit<MarketingProduct, "sku"> & { sku?: string }) =>
      fetchJSON<MarketingProduct>("/api/marketing/catalog", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(product),
      }),
    updateProduct: (sku: string, product: Partial<MarketingProduct>) =>
      fetchJSON<MarketingProduct>(`/api/marketing/catalog/${encodeURIComponent(sku)}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(product),
      }),
    deleteProduct: (sku: string) =>
      fetchJSON<{ ok: boolean }>(`/api/marketing/catalog/${encodeURIComponent(sku)}`, {
        method: "DELETE",
      }),
  },
  ...
  // ── HR ────────────────────────────────────────────────────────────────
  hr: {
    getJobs: () => fetchJSON<{ jobs: HRJob[] }>("/api/hr/jobs"),
    getJob: (id: string) => fetchJSON<HRJob>(`/api/hr/jobs/${encodeURIComponent(id)}`),
    createJob: (job: Omit<HRJob, "id">) =>
      fetchJSON<HRJob>("/api/hr/jobs", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(job),
      }),
    updateJob: (id: string, job: Partial<HRJob>) =>
      fetchJSON<HRJob>(`/api/hr/jobs/${encodeURIComponent(id)}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(job),
      }),
    deleteJob: (id: string) =>
      fetchJSON<{ ok: boolean }>(`/api/hr/jobs/${encodeURIComponent(id)}`, {
        method: "DELETE",
      }),
    postJob: (id: string, boards: string[]) =>
      fetchJSON<{ ok: boolean }>(`/api/hr/jobs/${encodeURIComponent(id)}/post`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ boards }),
      }),
    getPipeline: (jobId?: string) =>
      fetchJSON<{ applications: HRApplication[]; stages: string[] }>(
        `/api/hr/pipeline${jobId ? `?job_id=${encodeURIComponent(jobId)}` : ""}`,
      ),
    moveApplication: (applicationId: string, newStage: string) =>
      fetchJSON<{ ok: boolean }>("/api/hr/pipeline/move", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ application_id: applicationId, new_stage: newStage }),
      }),
    getCandidates: () =>
      fetchJSON<{ candidates: HRCandidate[] }>("/api/hr/candidates"),
    getCandidate: (id: string) =>
      fetchJSON<HRCandidate>(`/api/hr/candidates/${encodeURIComponent(id)}`),
    getCandidateCv: (id: string) =>
      authedFetch(`/api/hr/candidates/${encodeURIComponent(id)}/cv`),
    compareCandidates: (candidateIds: string[], jobId: string) =>
      fetchJSON<{ comparison: Record<string, unknown> }>("/api/hr/compare", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ candidate_ids: candidateIds, job_id: jobId }),
      }),
    screenCandidates: (jobId: string) =>
      fetchJSON<{ ok: boolean }>("/api/hr/screen", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ job_id: jobId }),
      }),
    getScheduleSlots: (eventTypeId: string, date: string) =>
      fetchJSON<{ slots: HRScheduleSlot[] }>(
        `/api/hr/schedule/slots?event_type_id=${encodeURIComponent(eventTypeId)}&date=${encodeURIComponent(date)}`,
      ),
    bookSchedule: (eventTypeId: string, start: string, candidateId: string) =>
      fetchJSON<{ ok: boolean; booking_url?: string }>("/api/hr/schedule/book", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ event_type_id: eventTypeId, start, candidate_id: candidateId }),
      }),
    getCommsLog: (candidateId?: string, channel?: string) =>
      fetchJSON<{ messages: HRCommsMessage[] }>(
        `/api/hr/comms/log${
          candidateId || channel
            ? `?${new URLSearchParams({
    ...
    sendComms: (candidateId: string, channel: string, text: string, template?: string) =>
      fetchJSON<{ ok: boolean }>("/api/hr/comms/send", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ candidate_id: candidateId, channel, text, template }),
      }),
    getAnalyticsOverview: (dateFrom?: string, dateTo?: string) => {
      const qs = new URLSearchParams();
      if (dateFrom) qs.set("date_from", dateFrom);
      if (dateTo) qs.set("date_to", dateTo);
      return fetchJSON<HRAnalyticsOverview>(`/api/hr/analytics/overview?${qs.toString()}`);
    },
  },
...
export interface MarketingPipelineResponse {
  items: MarketingPiece[];
  states: string[];
}
...
export interface MarketingHookLeaderboardEntry {
  hook: string;
  uses: number;
  avg_views: number;
}
...
export interface HRCommsMessage {
  id: string;
  candidate_id: string;
  channel: string;
  text: string;

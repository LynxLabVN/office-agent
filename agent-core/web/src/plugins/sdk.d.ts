...
export interface HermesPluginSDK {
   ...
   *
   * Namespaces (e.g. ``api.marketing.*``, ``api.hr.*``) are supported.
   */
  /**
   * Typed convenience client for core dashboard endpoints. Typed permissively
   * at the boundary (methods vary in arity and return type — most return
...
/** Recursive API client shape supporting flat methods and nested namespaces. */
export type ApiNamespace = Record<string, ((...args: never[]) => unknown) | ApiNamespace>;
